...
# register 3d waxholm image to 3d Lhx6 using affine
...
OUTPUT=/home/songgang/mnt/data/Histo/Yuko/preliminary_anatomical_new/output_histo2WHS/Lhx6_to_waxholm_dmmfd_
...
echo
echo $ANTS 3 -m MI[$FULLVOLMASK,$HISTMASK,1,32] \
                -t Elast[0.5] \
                -r DMFFD[4x4x4,3] \
                -i 100x100x100\
        -o $OUTPUT \
...
echo
echo $WARP 3 $HISTSLICE ${OUTPUT}warped_affined_histo.nii.gz -R $FULLVOLMASK ${OUTPUT}Affine.txt
echo $WARP 3 $HISTMASK ${OUTPUT}warped_affined_mask.nii.gz -R $FULLVOLMASK ${OUTPUT}Affine.txt --use-NN
echo
